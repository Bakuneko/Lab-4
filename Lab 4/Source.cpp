...
struct Node* CreateTree(struct Node* root, int data) {
    ...
    if (data < root->data) {
        root->left = CreateTree(root->left, data);  // Рекурсивно добавляем в левое поддерево
    }
    else if (data > root->data) {
        root->right = CreateTree(root->right, data);  // Рекурсивно добавляем в правое поддерево
    }
...
int main() {
    ...
    while (start) {
        ...
        if (D == -1) {
            ...
            root = CreateTree(root, D);  // Теперь правильно передаем только root
        }
    }
